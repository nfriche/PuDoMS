...
# Create pudoms folder if it doesn't exist
pudoms_folder = "pudoms"
os.makedirs(pudoms_folder, exist_ok=True)
...
for index, row in metadata.iterrows():
    try:
        ...
        if os.path.exists(original_path) and row['Split'] in ['train', 'validation', 'test']:
            ...
            # Define new paths for the pudoms folder
            new_path = os.path.join(pudoms_folder, midi_file)
            # Move MIDI file to the pudoms folder
            os.rename(original_path, new_path)
            print(f"Moved {midi_file} to {pudoms_folder}.")
            ...
            wav_path = os.path.join(pudoms_folder, wav_file)
            fs.midi_to_audio(new_path, wav_path)
            print(f"Converted {midi_file} to WAV and moved to {pudoms_folder}.")
        else:
